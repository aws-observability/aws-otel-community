...
namespace dotnet_sample_app.Controllers
{
    public class MetricEmitter
    {
        ...
        private KeyValuePair<string, object>[] requestAttributes = new KeyValuePair<string, object>[] {
        ...
        private KeyValuePair<string, object>[] randomAttributes = new KeyValuePair<string, object>[] {
                        new KeyValuePair<string, object>("signal", "metric"), 
                        new KeyValuePair<string, object>("language", "dotnet"), 
        ...
        public MetricEmitter()
        {
            ...
            meter.CreateObservableCounter(totalApiRequestSent,() => { 
                    return new Measurement<long>(apiRequestSent, requestAttributes);
                }, 
                "1",
                "Increments by one every time a sampleapp endpoint is used");

            meter.CreateObservableCounter(totalApiBytesSentMetricName, () => { 
                    return new Measurement<long>(totalBytesSent, requestAttributes);
                }, 
                "By",
            ...
            meter.CreateObservableGauge(cpuUsageMetricName, () => { 
                    return new Measurement<long>(cpuUsage, randomAttributes);
                }, 
                "1",
                "Cpu usage percent");

            meter.CreateObservableUpDownCounter(totalHeapSizeMetricName, () => { 
                    return new Measurement<long>(totalHeapSize, randomAttributes);
                }, 
                "1",
